...
- Name: 09_Frame_iFrame\240_SingleFrame.spec.ts >> select and access frames
- Location: tests\09_Frame_iFrame\240_SingleFrame.spec.ts:3:9
...
Call log:
  - waiting for locator('#vehicle-registration-card').contentFrame().getByTestId('#RESULT_TextField-1')
...
```ts
  ...
  2  | 
  3  |     test('select and access frames', async ({ page }) => {
  4  | 
  5  |         await page.goto('https://app.thetestingacademy.com/playwright/frames/');
  6  | 
> 7  |         await page.frameLocator('#vehicle-registration-card').getByTestId('#RESULT_TextField-1').fill('A232K')
     |                                                                                                  ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  8  |         
  9  | 
  10 | 
