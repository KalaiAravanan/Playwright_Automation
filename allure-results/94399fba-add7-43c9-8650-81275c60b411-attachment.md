...
Locator: locator('td.username')
Expected: "Kabir.Khan"
Error: strict mode violation: locator('td.username') resolved to 10 elements:
...
Call log:
  ...
  - waiting for locator('td.username')
...
```ts
  ...
  10 | 
  11 |     const usernameCell = page.locator('td.username');
> 12 |     await expect(usernameCell).toHaveText('Kabir.Khan');
     |                                ^ Error: expect(locator).toHaveText(expected) failed
  13 |     
  14 |    
  15 | });
